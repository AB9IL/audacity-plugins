...
(defun check-peak (peak)
  ...
  (if (> peak -1.00)
      "Fail (too high - Peaks must be no higher than -1 dB)"
      (if (< peak -4)
          (if (< peak -7)
              "Fail (too low - Peaks should be between -4 and -1 dB)"
                  "Warning (low - Peaks should be -4 to -1 dB.)")
      "Pass (-1 +0/- 3 dB)")))
...
(defun check-rms (rms)
  ...
  (if (> rms -14.00)
      (if (> rms -13.00)
      "Fail (too loud - RMS should range -16 to -14 dB)"
      "Warning (too loud RMS should range -16 to -14 dB)")
  (if (< rms -16.00)
          (if (< rms -17.00)
              "Fail (too quiet - RMS should range -16 to -14 dB)"
                  "Warning (too quiet - RMS should range -16 to -14 dB)")
      "Pass ( -15 +/- 1 dB)")))
