...
(defun check-peak (peak)
  ...
  (if (> peak -0.10)
      "Fail (too high - Peaks must be no higher than -0.1 dB)"
      (if (< peak -3.1)
          (if (< peak -6.1)
              "Fail (too low - Peaks should be between -3.1 and -0.1 dB)"
                  "Warning (low - Peaks should be -3.1 to -0.1 dB.)")
      "Pass (-0.1 +0/- 3 dB)")))
...
(defun check-rms (rms)
  ...
  (if (> rms -6.00)
      (if (> rms -4.50)
      "Fail (too loud - RMS should range -9 to -6 dB)"
      "Warning (too loud RMS should range -9 to -6 dB)")
  (if (< rms -9)
          (if (< rms -10.5)
              "Fail (too quiet - RMS should range -9 to -6 dB)"
                  "Warning (too quiet - RMS should range -9 to -6 dB)")
      "Pass ( -7.5 +/- 1.5 dB)")))
