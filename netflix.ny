      ...
      (if (< peak -5)
          (if (< peak -8)
              "Fail (too low - Peaks should be between -5 and -2 dB)"
                  "Warning (low - Peaks should be -5 to -2 dB.)")
      "Pass (-2 +0/- 3 dB)")))
...
(defun check-rms (rms)
  ...
  (if (> rms -25.00)
      (if (> rms -24.00)
      "Fail (too loud - RMS should range -29 to -25 dB)"
      "Warning (too loud RMS should range -29 to -25 dB)")
  (if (< rms -29.00)
          (if (< rms -30.00)
              "Fail (too quiet - RMS should range -29 to -25 dB)"
                  "Warning (too quiet - RMS should range -29 to -25 dB)")
      "Pass ( -27 +/- 2 dB)")))
